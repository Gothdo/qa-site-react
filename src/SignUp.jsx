...
class SignUp extends React.Component {
  ...
  handleSubmit = async (event) => {
    ...
    try {
      await axios.post('http://localhost:3000/user/register', this.state.form);
      const { headers } = await axios.post(
        'http://localhost:3000/security/login',
        R.dissoc('displayName', this.state.form),
      );
      localStorage.setItem('jwt', headers['x-auth-token']);
      this.props.history.push('/');
      this.props.onMessage(messages.accountCreated);
    } catch (axiosError) {
      ...
      <Card className={classes.root}>
        <CardContent>
          <form spellCheck="false" onSubmit={this.handleSubmit}>
            ...
            <Button type="submit" variant="raised" color="primary" className={classes.button}>Sign up</Button>
          </form>
        </CardContent>
      </Card>
    );
